...
function showModal(title, content, allow_close) {
    ...
    if (allow_close)
        ...
        (0, jquery_1.default)('#modal_close').hide();
 ...
 */
function newGame() {
    game.rows = (0, jquery_1.default)('#rows').val();
    game.cols = (0, jquery_1.default)('#cols').val();
    game.num_mines = (0, jquery_1.default)('#mmines').val();
...
function checkIfWon() {
    if (!game.lost && !game.won) {
        if ((0, jquery_1.default)('.mine_cell:not(.hollow)').length == game.num_mines) {
            game.won = true;
            alertModal('<h3>You Won!</h3>');
        }
        else if (parseInt((0, jquery_1.default)('#mines').text()) == 0) {
            game.won = true;
            (0, jquery_1.default)('.mine_cell:not(.hollow)').each(function () {
