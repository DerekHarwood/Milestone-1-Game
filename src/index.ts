 ...
 */
 //@ts-ignore
 $.fn.bindFirst = function(name:any, fn:any): void {
 ...
 * @return {boolean}
 */
 function alertKeydown(e:any): boolean {
 ...
 */
 ...
 function showModal(title:any, content:any, allow_close:any): void {
 ...
 * @return {null}
 */
 function hideModal(): void {//@ts-ignore
 ...
 * @return {null}
 */
 function newGame(): void {//@ts-ignore
    game.rows = $('#rows').val();//@ts-ignore
    game.cols = $('#cols').val();//@ts-ignore
    game.num_mines = $('#mmines').val();//@ts-ignore
 ...
 * @return {null}
 */
 function cellClick(this: any): void {
    if (!time_interval)//@ts-ignore
 ...
 */
 function cellRightClick(this: any, e:any): boolean {
    if (!time_interval)//@ts-ignore
 ...
 * @return {null}
 */
function updateTime(): void {//@ts-ignore
    if (!game.lost && !game.won)
 ...
 * @return {null}
 */
function checkIfWon(): void {//@ts-ignore
    if (!game.lost && !game.won) {
        if ($('.mine_cell:not(.hollow)').length == game.num_mines) {//@ts-ignore
            game.won = true;
            alertModal('<h3>You Won!</h3>');
        } else if (parseInt($('#mines').text()) == 0) {//@ts-ignore
            game.won = true;
            $('.mine_cell:not(.hollow)').each(function() {
                if (typeof($(this).data('flag')) != 'undefined' && $(this).data('flag') > 0)
                    if ($(this).data('cell') != -1)//@ts-ignore
                        game.won = false;
            });//@ts-ignore
            if (game.won)
                alertModal('<h3>You Won!</h3>');
